...
/obj/item/weapon/sword/long/greatsword/copperclaymore
	name = "copper claymore"
	desc = "A large copper claymore, its cheap and crude compared to other claymores."
	icon_state = "copper_claymore"
	force_wielded = DAMAGE_POOR_LONGSWORD_WIELD
	possible_item_intents = list(SWORD_CUT, SWORD_THRUST, SWORD_STRIKE)
	gripped_intents = list(SWORD_CUT, SWORD_THRUST, SWORD_STRIKE, SWDLONG_CHOP)
	max_blade_int = 100 // Copper tier
	max_integrity = INTEGRITY_ABOVE_AVERAGE
	minstr = 8
	sellprice = 40

/obj/item/weapon/sword/long/greatsword/copperclaymore/getonmobprop(tag)
	. = ..()
	if(tag)
		switch(tag)
			if("gen")
